...
if (options && options.onResponseHeader) options.onResponseHeader(response);
        ...
        deleteOrder(orderId: string, options?: any): (fetch?: FetchAPI, basePath?: string) => Promise<any> {
            const localVarFetchArgs = OrdersServiceApiFetchParamCreator(configuration).deleteOrder(orderId, options);
            return (fetch: FetchAPI = portableFetch, basePath: string = BASE_PATH) => {
                return fetch(basePath + localVarFetchArgs.url, localVarFetchArgs.options).then((response) => {
                    if (response.status >= 200 && response.status < 300) {
if (options && options.onResponseHeader) options.onResponseHeader(response);
...
export const OrdersServiceApiFactory = function (configuration?: Configuration, fetch?: FetchAPI, basePath?: string) {
    return {
        ...
        deleteOrder(orderId: string, options?: any) {
            return OrdersServiceApiFp(configuration).deleteOrder(orderId, options)(fetch, basePath);
        },
        /**
         * 
         * @param {string} customerId The field will contain id of the resource requested.
...
export class OrdersServiceApi extends BaseAPI {
    ...
    public deleteOrder(orderId: string, options?: any) {
        return OrdersServiceApiFp(this.configuration).deleteOrder(orderId, options)(this.fetch, this.basePath);
    }
